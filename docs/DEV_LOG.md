...
With those in place TP=1 and TP=2 worked and **TP>=4 wedged on every model**:
engine init finished completely -- weights, KV cache, cudagraph capture -- then
the first forward pass hung forever in
`[MXKW][E] queues.c:844 mxkwCreateQueueBlock ioctl create queue block timeout ... Retrying`.
Every one of those lines is unprefixed, i.e. emitted by the driver process, not
by a worker.

Ruled out, each by experiment: process count (four concurrent processes each
running InfiniCore ops on their own card are fine), the C++ bridge
(`VLLM_INFINICORE_DISABLE_CPP_BRIDGE=1` still wedges), cudagraph
(`--execution-mode eager` still wedges), NCCL channel budget
(`NCCL_MAX_NCHANNELS=1` still wedges), memory pressure
(`--gpu-memory-utilization 0.5` still wedges), and ray (installed 2.58.0; still
wedges). Route-subset bisection is impossible on the strict platform: any partial
route set fails independently with `NameError: reshape_and_cache_flash`, because
there is no `vllm_metax` to supply that fallback, so routes are all-or-nothing.

Root cause is in InfiniCore, not this plugin. `ContextImpl::ContextImpl()` in
`src/infinicore/context/context_impl.cc` built its default `Runtime` on device
index **0**, hardcoded. Every tensor-parallel worker therefore opened a context on
card 0 on top of the card it actually used. Counting contexts with `mx-smi`
during a run:

| Engine | TP | contexts on card 0 | total | Result |
|---|---:|---:|---:|---|
| vllm-metax | 4 | 2 | 5 | runs |
| vllm-infinicore (before) | 4 | 4 | 7 | wedges |
| vllm-infinicore (after) | 4 | 2 | 5 | runs |

Card 0 tops out at four contexts, so the driver process's own context would have
been the fifth, and creating it is what hung. TP<=2 stayed under the limit, which
is exactly why only TP>=4 failed.

Note that `ContextImpl::setDevice()` calls `getCurrentRuntime()` before switching,
and that lazily builds the default card-0 runtime purely to answer a comparison.
Calling `infinicore.set_device()` early therefore does **not** avoid the stray
context; that was tried and measured to have no effect.

The fix has two halves and needs both. In InfiniCore,
`patches/infinicore-default-device-index.patch` adds a `default_device_index()`
honouring `INFINICORE_DEFAULT_DEVICE_INDEX`, makes `getCurrentRuntime()` scan the
row for the first non-null runtime rather than assume index 0, and stops
`setDevice()` calling `getCurrentRuntime()` merely to compare. In this plugin,
`set_device` exports `INFINICORE_DEFAULT_DEVICE_INDEX` for the worker's own card
-- the earliest per-worker hook that still precedes the first InfiniCore call.
With the variable unset InfiniCore behaves exactly as before, so the patch is
backward compatible. Rebuild with
`PATH=/mnt/geogpt-doc-new/default/xmake_env/bin:$PATH XMAKE_ROOT=y xmake build && xmake install`
(about 60s incremental, installs into `$INFINI_ROOT`).
...
InfiniCore holds 69-86% of vLLM-MetaX, and the ratio falls monotonically with TP
on every model, so the gap is in multi-card scaling and not only in single-card
operator speed. It narrows as the model grows, which is what fixed per-step
overhead being amortised looks like.
